program PowerReconstr
    ...
    data B2 / 3.19254, -13.7509, 0., -6.67985/ ! NG = 2
...
subroutine MAT_Set_MatFunc_ANM_1D(xappa, Matr_Func)
    ...
    a1(3) = (F_01_03(gamma(1),eigenv(1)) &
        - F_01_03(gamma(2),eigenv(2)))/denom
    ...
    a2(3) = (-Eigenv(2)*F_01_03(gamma(1),eigenv(1)) &
        + Eigenv(1)*F_01_03(gamma(2),eigenv(2)))/denom
    ...
    contains
        ...
        real function F_01_03(gamma,eigenv)
            real gamma, eigenv

            F_01_03 = gamma/eigenv
...
subroutine MAT_Set_MatFunc_Sin_ANM(xappa, x, Matr_Func_Sin)
    ...
    do n = 1, NG
        if(Eigenv(n).LT.0) then
            gamma(n) = sin(eigenv(n)*x)
        else
            gamma(n) = sinh(eigenv(n)*x)
        end if
    end do
...
subroutine MAT_Set_MatFunc_Cos_ANM(xappa, x, Matr_Func_Cos)
    ...
    do n = 1, NG
        if(Eigenv(n).LT.0) then
            gamma(n) = cos(eigenv(n)*x)
        else
            gamma(n) = cosh(eigenv(n)*x)
        end if
    end do
